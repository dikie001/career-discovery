...
export function ConsentModal({ isOpen, onConsent, isLoading = false }: ConsentModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="w-full max-w-md mx-4 rounded-2xl bg-gradient-to-br from-card to-background border border-border shadow-2xl animate-slideUp">
        {/* Header */}
        <div className="flex items-start justify-between p-6 border-b border-border">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">Enhance your career guidance experience</p>
            </div>
          </div>
        </div>
        ...
        <div className="px-6 py-4 border-t border-border">
          <label className="flex items-center gap-3 cursor-pointer group">
            <input
              ...
              disabled={isLoading}
              className="w-4 h-4 rounded border-input bg-muted text-teal-600 focus:ring-2 focus:ring-teal-500 cursor-pointer disabled:opacity-50"
            />
        ...
        <div className="flex gap-3 p-6 border-t border-border bg-card/50">
          <button
            ...
            disabled={isLoading}
            className="flex-1 px-4 py-2.5 rounded-lg border border-border text-muted-foreground hover:text-white hover:bg-muted transition-all disabled:opacity-50 font-medium text-sm"
          >
            Maybe Later
          </button>
          <button
